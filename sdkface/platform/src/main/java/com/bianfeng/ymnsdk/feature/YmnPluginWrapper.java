...
public abstract class YmnPluginWrapper extends YmnPlugin {
    ...
    public void checkState(PluginLocalState local) {
        switch (getPolicy()) {
            case LAZY:
                ...
                break;
            case REMOTE:
                state = STATE_PENDING_CHECK;
                break;
            case FORCE:
                state = STATE_WORKING;
                break;
            case TRIGGER:
                if (triggered) state = STATE_WORKING;
    ...
    public boolean isCheckedState() {
        return state != STATE_EMPTY && state != STATE_PENDING_CHECK;
    }
    ...
    public String toString() {
        return String.format("%s {%s, inited = %b, initing = %b, class = %s}", getPluginName(), STATE_NAME.get(state), inited, initing, getClass().getName());
    }
